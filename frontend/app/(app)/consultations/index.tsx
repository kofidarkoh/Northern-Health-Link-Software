...
export default function ConsultationsScreen() {
  ...
  const { data, isLoading, refetch } = useQuery({
    ...
    queryFn: () =>
      clinicalApi.listAppointments({
        ...
        specialist_id: hasRole('SPECIALIST') ? user?.id : undefined,
      }),
  ...
  const statusCounts = statuses.reduce((acc, s) => { acc[s] = appointments.filter(a => a.status === s).length; return acc }, {} as Record<string, number>)

  if (isLoading) {
    return (
      <Screen>
        <PageHeader title="Consultations" showBack={false} />
  ...
  return (
    <Screen>
      <PageHeader title="Consultations" subtitle="Specialist evaluations & notes" showBack={false} />
      ...
      >
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.chipScroll}>
          <Chip selected={statusFilter === ''} onPress={() => setStatusFilter('')} style={styles.chip} selectedColor={Colors.primary} showSelectedOverlay>All</Chip>
          {statuses.map(s => (
            <Chip key={s} selected={statusFilter === s} onPress={() => setStatusFilter(s)} style={styles.chip} selectedColor={STATUS_CONFIG[s].color} showSelectedOverlay>
              ...
              <View key={appt.id} style={styles.card}>
                ...
                <View style={styles.cardBody}>
                  <View style={styles.cardTop}>
                    <View style={styles.cardLeft}>
                      ...
                      <Text style={styles.cardTime}>
                        {new Date(appt.appointment_time).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
                      </Text>
                    </View>
                    <View style={[styles.statusBadge, { backgroundColor: cfg.color + '15' }]}>
                      <Ionicons name={cfg.icon as any} size={12} color={cfg.color} />
                      <Text style={[styles.statusText, { color: cfg.color }]}>{appt.status.replace('_', ' ')}</Text>
                    </View>
                  </View>
                  <View style={styles.cardMeta}>
                    ...
                    <View style={[styles.metaBadge, { backgroundColor: '#17A2B8' + '12' }]}>
                      <Ionicons name="business" size={10} color="#17A2B8" />
                      <Text style={[styles.metaText, { color: '#17A2B8' }]}>Clinic #{appt.clinic_id}</Text>
                    </View>
                  </View>
                </View>
              </View>
...
const styles = StyleSheet.create({
  ...
  statCard: { flex: 1, backgroundColor: Colors.surface, borderRadius: 12, padding: Spacing.sm, alignItems: 'center', borderWidth: 1, borderColor: Colors.border },
